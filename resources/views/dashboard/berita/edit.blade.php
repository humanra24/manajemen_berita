    ...
    <div class="row justify-content-center">
        <div class="col-12 col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('berita.update', ['beritum'=>$data['berita']->berita_id]) }}" method="POST">
                        @method('PUT')
                        @csrf
                        <div class="mb-3">
                            <label for="judulBerita" class="form-label">Judul Berita</label>
                            <input type="text" class="form-control @error('judul_berita') is-invalid @enderror" id="judulBerita" name="judul_berita" value="{{ old('judul_berita', $data['berita']->judul_berita) }}" />
                            @error('judul_berita')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="selected form-select  @error('kategori') is-invalid @enderror" name="kategori">
                                ...
                                @forelse ($data['kategori'] as $kategori)
                                    <option @if (old('kategori', $data['berita']->kategori_id) == $kategori->kategori_id) selected @endif value="{{ $kategori->kategori_id }}">{{ $kategori->kategori }}</option>
                                @empty
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="isi_berita" class="form-control @error('isi_berita') is-invalid @enderror" id="" rows="5">{{ old('isi_berita', $data['berita']->isi_berita) }}</textarea>
                            @error('isi_berita')
